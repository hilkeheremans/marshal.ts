...
import { CustomError, isPlainObject } from '@thinman/marcj-estdlib';
import { getClassSchema, f } from '../src/decorators';

test('test simple', async () => {
  class Page {
    constructor(@f public name: string, @f public age: number) {}
...
test('test nested validation', async () => {
  ...
  ).toEqual([{ message: 'Type is not an object', code: 'invalid_type', path: 'nested' }]);
...
test('test valdiation on real life case', () => {
  ...
  class NodeGpuResource {
    ...
    constructor(@f.asName('uuid') public uuid: string, @f.asName('name') public name: string) {}
  }
  ...
  expect(
    isPlainObject(object.assignedJobTaskInstances['a09be358-d6ce-477f-829a-0dc27219de34.0.main'].assignedResources),
  ).toBeTrue();
