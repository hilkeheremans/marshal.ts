...
import { getEnumLabels, getEnumValues, getValidEnumValue, isValidEnumValue } from '@thinman/marcj-estdlib';
...
test('test simple model all fields', () => {
  const instance = plainToClass(SimpleModel, {
    ...
    children: [{ label: 'fooo' }, { label: 'barr' }],
    childrenMap: {
      foo: {
        label: 'bar',
      },
...
test('test @Decorated with parent', async () => {
  ...
  const instance = plainToClass(DocumentClass, {
    ...
    page: {
      name: 'RootPage',
      children: [{ name: 'RootPage.1' }, { name: 'RootPage.2' }, { name: 'RootPage.3' }],
    },
    pages: [
      {
        name: 'Foo',
        children: [
          {
            name: 'Foo.1',
            children: [{ name: 'Foo.1.1' }, { name: 'Foo.1.2' }, { name: 'Foo.1.3' }],
          },
        ],
      },
      { name: 'Bar' },
...
test('enums', () => {
  ...
  expect(
    ...
    {
      code: 'invalid_enum',
      message: 'Invalid enum value received. Allowed: 0,1',
      path: 'enum1',
    },
  ]);
  ...
  expect(
    ...
    {
      code: 'invalid_enum',
      message: 'Invalid enum value received. Allowed: 200,x,first,second',
      path: 'enumLabels',
    },
  ]);
  ...
  expect(
    ...
    {
      code: 'invalid_enum',
      message: 'Invalid enum value received. Allowed: 200,100',
      path: 'enum3',
    },
  ]);
...
test('enums arrays', () => {
  ...
  ).toEqual([{ code: 'invalid_enum', message: 'Invalid enum value received. Allowed: z,x', path: 'enum2.0' }]);
});
