...
import { ClassType, getClassName, getClassPropertyName } from '@thinman/marcj-estdlib';
...
export class CacheJitPropertyConverter {
  ...
  constructor(public readonly fromFormat: string, public readonly toFormat: string) {}
...
export class JitPropertyConverter {
  ...
  constructor(public readonly fromFormat: string, public readonly toFormat: string, classType: ClassType<any>) {
  ...
  return `
    ...
    if (!${setter})
        throw new Error('${getClassPropertyName(classType, property.name)} is defined as @ParentReference() and ' +
                    'NOT @f.optional(), but no parent found. Add @f.optional() or provide ${
                      property.name
                    } in parents to fix that.');
    `;
}
...
export function createClassToXFunction<T>(classType: ClassType<T>, toFormat: string | 'plain'): (instance: T) => any {
  if (toFormat === 'plain') {
...
export function createXToClassFunction<T>(
  classType: ClassType<T>,
  fromTarget: string | 'plain',
  ...
  for (const property of constructorParameter) {
    ...
    if (schema.decorator && property.name === schema.decorator) {
      ...
      constructorArguments.push(
        `var c_${property.name}; ` + getParentResolverJS(classType, `c_${property.name}`, property, context),
      );
    } else {
