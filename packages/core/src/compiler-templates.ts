...
registerConverterCompiler(
  ...
  'class',
  'date',
  (setter: string, accessor: string, property: PropertyCompilerSchema) => {
...
registerConverterCompiler(
  ...
  'class',
  'enum',
  (setter: string, accessor: string, property: PropertyCompilerSchema, reserveVariable) => {
...
export function compilerXToUnionClass(fromFormat: string) {
  return (setter: string, accessor: string, property: PropertyCompilerSchema, reserveVariable) => {
    ...
    for (const type of property.resolveUnionTypes) {
      ...
      discriminator += `if (${accessor}.${discriminant.name} === ${JSON.stringify(
        discriminant.defaultValue,
      )}) ${discriminatorClassVarName} = ${typeVarName};\n`;
    }
