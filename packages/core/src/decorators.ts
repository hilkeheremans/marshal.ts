...
export class PropertyCompilerSchema {
  ...
  constructor(public name: string, protected classType?: ClassType<any> | ClassType<any>[]) {}
  ...
  static createFromPropertySchema(
    ...
    const i = new PropertyCompilerSchema(propertySchema.name, propertySchema.getResolvedClassTypeForValidType());
...
export class PropertySchema extends PropertyCompilerSchema {
  ...
  toJSON(): PropertySchemaSerialized {
    ...
    if (resolved) {
      ...
      if (!name) {
        throw new Error(
          `Could not serialize type information for ${this.methodName ? this.methodName + ':' : ''}${
            this.name
          }, got type ${getClassName(resolved)}. ` +
            `Either further specify the type using the @f decorator or use @Entity() decorator at the ${getClassName(
              resolved,
            )} class.`,
        );
      }
  ...
  static fromJSON(props: PropertySchemaSerialized): PropertySchema {
    ...
    if (props['classType']) {
      ...
      if (!entity) {
        throw new Error(
          `Could not unserialize type information for ${p.methodName || ''}:${p.name}, got entity name ${
            props['classType']
  ...
  setFromJSType(type: any) {
    ...
    const isCustomObject = !typedArrayMap.has(type) && type !== 'any' && type !== Object;
    if (isCustomObject) {
      this.type = 'class';
  ...
  getRequiredForeignFieldsForProxy(): PropertySchema[] {
    return [
      this.getResolvedClassSchema().getPrimaryField(),
      ...this.getResolvedClassSchema().getMethodProperties('constructor'),
    ];
  }
  ...
  getResolvedClassType(): ClassType<any> {
    ...
    if (!this.classType || isArray(this.classType)) {
      throw new Error(
        `No ClassType given for field ${this.name}. Use @f.forward(() => MyClass) for circular dependencies.`,
      );
    }
...
export class ClassSchema<T = any> {
  ...
  protected initializeMethod(name: string) {
    if (!this.initializedMethods[name]) {
      if (!Reflect.hasMetadata('design:returntype', this.classType.prototype, name)) {
        throw new Error(
          `Method ${name} has no decorators used, so reflection does not work. Use @f on the method or arguments.`,
        );
      }
  ...
  public findReverseReference(toClassType: ClassType<any>, fromReference: PropertySchema): PropertySchema {
    ...
    for (const backRef of this.references) {
      ...
      if (
        fromReference.backReference &&
        fromReference.backReference.via &&
        backRef.backReference &&
        backRef.backReference.via
      ) {
        if (
          resolveClassTypeOrForward(fromReference.backReference.via) ===
          resolveClassTypeOrForward(backRef.backReference.via)
        ) {
    ...
    if (candidates.length > 1) {
      throw new Error(
        `Class ${getClassName(this.classType)} has multiple potential reverse references [${candidates
          .map((v) => v.name)
          .join(', ')}] for ${fromReference.name} to class ${getClassName(toClassType)}. ` +
    ...
    throw new Error(
      `Class ${getClassName(this.classType)} has no reference to class ${getClassName(toClassType)} defined.`,
    );
  }
...
export function getClassSchema<T>(classTypeIn: ClassType<T> | Object): ClassSchema<T> {
  const classType = classTypeIn['prototype']
...
export interface FieldDecoratorResult<T> {
  ...
  validator(
    validator:
      ...
      | ((value: any, target: ClassType<any>, propertyName: string) => PropertyValidatorError | void),
  ): this;
}
...
function createFieldDecoratorResult<T>(
  ...
  const fn = (target: Object, propertyOrMethodName?: string, parameterIndexOrDescriptor?: any) => {
    ...
    const isMethod =
      ...
      Reflect.hasMetadata('design:returntype', target, propertyOrMethodName) &&
      !isNumber(parameterIndexOrDescriptor);
    ...
    if (isNumber(parameterIndexOrDescriptor)) {
      ...
      if (
        givenPropertyName &&
        ((methodsParamNames[parameterIndexOrDescriptor] &&
          methodsParamNames[parameterIndexOrDescriptor] !== givenPropertyName) ||
          (methodsParamNamesAutoResolved[parameterIndexOrDescriptor] &&
        ...
        throw new Error(
          `Defining multiple Marshal decorators with different names at arguments of ${getClassName(
            target,
          )}::${methodName} #${parameterIndexOrDescriptor} is forbidden.` +
            ` @f.asName('name') is required. Got ${
              methodsParamNames[parameterIndexOrDescriptor] || methodsParamNamesAutoResolved[parameterIndexOrDescriptor]
            } !== ${givenPropertyName}`,
        );
      }
  ...
  fn.validator = (validator: ClassType<PropertyValidator> | ((value: any) => PropertyValidatorError | void)) => {
    ...
    const validatorClass: ClassType<PropertyValidator> = isPropertyValidator(validator)
      ? validator
      : class implements PropertyValidator {
          validate<T>(value: any): PropertyValidatorError | void {
            return validator(value);
          }
        };

    return createFieldDecoratorResult(
      cb,
      givenPropertyName,
      [
        ...modifier,
        (target: Object, property: PropertySchema) => {
...
function Field(...oriType: [FieldTypes | Types] | (ClassType<any> | ForwardedRef<any>)[]) {
  return FieldDecoratorWrapper((target, property, returnType, options) => {
    ...
    let type: FieldTypes | Types | (ClassType<any> | ForwardedRef<any>)[] =
      oriType.length <= 1 ? (oriType[0] as any) : oriType;
    ...
    if (!isArray(type) && returnType !== Promise && returnType !== undefined && type !== 'any') {
      ...
      if (type && options.array && returnType !== Array) {
        throw new Error(
          `${id} type mismatch. Given ${getTypeDeclaration(type, options)}, but declared is ${getTypeName(
            returnType,
      ...
      if (!type && returnType === Array) {
        throw new Error(
          `${id} type mismatch. Given nothing, but declared is Array. You have to specify what type is in that array.  ` +
      ...
      if (!type && returnType === Object) {
        ...
        throw new Error(
          `${id} type mismatch. Given ${getTypeDeclaration(type, options)}, but declared is Object or undefined. ` +
    ...
    const isCustomObject =
      !typedArrayMap.has(type) && type !== Object && typeof type !== 'string' && !(type instanceof ForwardedRef);
    ...
    if (type && !isArray(type) && !options.map && !options.partial && isCustomObject && returnType === Object) {
      ...
      throw new Error(
        `${id} type mismatch. Given ${getTypeDeclaration(type, options)}, but declared is Object or undefined. ` +
          ...
          `Please declare a type or use @f.map(${getClassName(
            type,
          )}) for '${propertyName}: {[k: string]: ${getClassName(type)}}'.`,
      );
    }
...
fRaw['array'] = function <T extends FieldTypes>(
  this: FieldDecoratorResult<any>,
  ...type: [T] | (ClassType<any> | ForwardedRef<any>)[]
): FieldDecoratorResult<T> {
  return Field(...type).asArray();
};

fRaw['map'] = function <T extends FieldTypes>(
  this: FieldDecoratorResult<any>,
  ...type: [T] | (ClassType<any> | ForwardedRef<any>)[]
): FieldDecoratorResult<T> {
...
fRaw['union'] = function <T>(
  this: FieldDecoratorResult<any>,
  ...types: (ClassType<any> | ForwardedRef<any>)[]
): FieldDecoratorResult<T> {
  return Field(...types);
};

fRaw['discriminant'] = function <T extends FieldTypes>(
  this: FieldDecoratorResult<any>,
  type: T,
): FieldDecoratorResult<T> {
...
fRaw['enum'] = function <T>(
  this: FieldDecoratorResult<T>,
  clazz: T,
...
fRaw['forward'] = function <T extends ClassType<any>>(
  this: FieldDecoratorResult<T>,
  f: () => T,
): FieldDecoratorResult<T> {
...
fRaw['forwardArray'] = function <T extends ClassType<any>>(
  this: FieldDecoratorResult<T>,
  f: () => T,
): FieldDecoratorResult<T> {
...
fRaw['forwardMap'] = function <T extends ClassType<any>>(
  this: FieldDecoratorResult<T>,
  f: () => T,
): FieldDecoratorResult<T> {
...
fRaw['forwardPartial'] = function <T extends ClassType<any>>(
  this: FieldDecoratorResult<T>,
  f: () => T,
): FieldDecoratorResult<T> {
