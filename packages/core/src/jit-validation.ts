import { ClassType } from '@thinman/marcj-estdlib';
...
export function getDataCheckerJS(
  ...
  const requiredThrown = property.isActualOptional()
    ? ''
    : `_errors.push(new ValidationError(${path}, 'required', 'Required value is undefined or null'));`;
  ...
  if (property.isArray) {
    ...
    const checkItem = compiler
      ? executeCheckerCompiler(`${path} + '.' + l`, rootContext, compiler, `${accessor}[l]`, property)
      : '';
    ...
    const line = compiler
      ? executeCheckerCompiler(`${path} + '.' + i`, rootContext, compiler, `${accessor}[i]`, property)
      : ``;
    return `
...
export function jitValidateProperty(
  property: PropertyCompilerSchema,
): (value: any, path?: string, errors?: ValidationError[], overwritePah?: string) => ValidationError[] {
  ...
  const notOptionalCheckThrow =
    property.isActualOptional() || property.hasDefaultValue
      ? ''
...
export function jitValidate<T>(
  ...
  for (let property of schema.classProperties.values()) {
    ...
    const notOptionalCheckThrow =
      property.isActualOptional() || property.hasDefaultValue
        ? ''
        : `_errors.push(new ValidationError(_path + '${property.name}', 'required', 'Required value is undefined or null'));`;
...
export function jitValidatePartial<T, K extends keyof T>(
  ...
  for (const i in partial) {
    ...
    jitValidateProperty(schema.classProperties.get(i) || resolvePropertyCompilerSchema(schema, i))(
      partial[i],
      '',
      errors,
      thisPath,
